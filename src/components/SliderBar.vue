...
const handleSliderMouseDown = (e: MouseEvent) => {
  ...
  const { offsetX } = e
  const percentage = calcPercentage(offsetX)
...
const handleMouseOut = (evt: any) => {
  ...
  const to = evt!.relatedTarget || evt!.toElement
  ...
  <div ref="target" class="group py-1 no-select" @mousedown="handleSliderMouseDown">
    <div
      :style="{
        width: width + 'px',
        height: height + 'px',
        transform: `scaleY(${isTargetClick || isHover ? '1.5' : '1'})`
      }"
