// copy by https://github.com/xiong35/image-color-extraction/blob/main/src/ColorExtractor.ts
import { chooseReadableColor } from './colors'
...
export class ColorExtractor {
  ...
  setConfig(config: Partial<Config>) {
    ...
    if (config.compresionRate !== undefined && config.compresionRate < 0) throw 'compresion rate must not be negative'
    if (config.timeout !== undefined && typeof config.timeout !== 'number') throw 'timeout must be number'
  ...
  async extractColor(image: string | HTMLImageElement): Promise<ColorInfo[] | undefined> {
   ...
   */
  chooseReadableColor() {
    if (!this.colors) throw "you haven't extracted colors, please call `extractColor` first"

    if (this.colors.length < 2) throw 'cannot generete two colors because there is only one color in this image'
